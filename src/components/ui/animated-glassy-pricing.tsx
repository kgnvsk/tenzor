...
export const PricingCard = ({
  ...
  const cardClasses = `
    ...
    ${isPopular ? 'scale-105 relative ring-2 ring-orange-400/20 dark:from-white/20 dark:to-white/10 dark:border-orange-400/30 shadow-2xl' : ''}
  `;
  const buttonClasses = `
    mt-auto w-full py-2.5 rounded-xl font-semibold text-[14px] transition font-sans
    ${buttonVariant === 'primary' 
      ? 'bg-orange-400 hover:bg-orange-300 text-foreground' 
  ...
  return (
    <div className={cardClasses.trim()}>
      {isPopular && (
        <div className="absolute -top-4 right-4 px-3 py-1 text-[12px] font-semibold rounded-full bg-orange-400 text-foreground dark:text-black">
          Most Popular
        </div>
      )}
      <div className="mb-3">
      ...
      <ul className="flex flex-col gap-2 text-[14px] text-foreground/90 mb-6 font-sans">
        {features.map((feature, index) => (
          <li key={index} className="flex items-center gap-2">
            <CheckIcon className="text-orange-400 w-4 h-4" /> {feature}
          </li>
        ))}
      </ul>
  ...
  return (
    <div className="relative bg-background text-foreground min-h-screen w-full overflow-hidden">
      ...
      <main className="relative w-full min-h-screen flex flex-col items-center justify-center px-4 py-8 z-10">
        <div className="w-full max-w-5xl mx-auto text-center mb-14 mt-16 xl:-mt-32 relative z-20">
